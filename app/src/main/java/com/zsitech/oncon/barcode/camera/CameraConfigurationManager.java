 ...
 */
package com.zsitech.oncon.barcode.camera;
...
final class CameraConfigurationManager {
    ...
    private static final int MAX_PREVIEW_PIXELS = 1280 * 730;
    ...
    void setDesiredCameraParameters(Camera camera, boolean safeMode) {
        ...
        if (parameters == null) {
            Log.w(TAG,
                    "Device error: no camera parameters are available. Proceeding without " +
                            "configuration.");
            return;
        }
        ...
        if (!safeMode && focusMode == null) {
            focusMode = findSettableValue(parameters.getSupportedFocusModes(),
                    Camera.Parameters.FOCUS_MODE_MACRO, "edof"); // Camera.Parameters
            // .FOCUS_MODE_EDOF
            // in 2.2+
        }
        if (focusMode != null) {
    ...
    private Point findBestPreviewSizeValue(Camera.Parameters parameters,
        ...
        Point bestSize = null;
        float screenAspectRatio = (float) screenResolution.x//横竖比例
                / (float) screenResolution.y;
        float diff = Float.POSITIVE_INFINITY;
        for (Camera.Size supportedPreviewSize : supportedPreviewSizes) {
            int realWidth = supportedPreviewSize.width;
            int realHeight = supportedPreviewSize.height;
            int pixels = realWidth * realHeight;//屏幕总像素乘积
            boolean isCandidatePortrait = true;//是否竖屏
            //            boolean isCandidatePortrait = realWidth < realHeight;//是否竖屏
